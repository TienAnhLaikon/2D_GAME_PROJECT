...
bool FireCombust::init() {
    skillTalent = new SkillTalent;
    skillTalent->skillName = "FireCombust";
    skillTalent->iconPath = "skill/SkillSprite/SkillIcon/Fire/fire_combust.png";
    skillTalent->unlockPoint = 30;
    skillTalent->upgradePoint = 30;
    skillTalent->description = "Summor a fire storm, dealing damage to each enemy hit by the ability within the affected area.";
    skillTalent->skillType = "AOE & Target";
    skillTalent->isLock = true;
    skillTalent->parent = nullptr;
    skillTalent->levelRequired = 0;
    this->skillName = "FireCombust";
    applyRange = 95;
    //AOE Range
    ...
    //Skill Sprite
    _skillSprite = Sprite::createWithSpriteFrameName("fire_combust (1).png");
    _skillSprite->setScale(0.1);
    _skillSprite->setAnchorPoint(Vec2(0.5, 0.25));
    _skillSprite->retain();

    //Skill Animate
    _skillAnimate = Animate::create(Engine::createAnimation2("fire_combust", 60, 0.05));
    ...
    touchListener->onTouchBegan = CC_CALLBACK_2(FireCombust::onTouchBegan, this);
    touchListener->onTouchMoved = CC_CALLBACK_2(FireCombust::onTouchMoved, this);
    touchListener->onTouchEnded = CC_CALLBACK_2(FireCombust::onTouchEnded, this);
...
bool FireCombust::onTouchBegan(Touch* touch, Event* event)
{
    ...
    if (_skillButton->skillButtonBtn->getBoundingBox().containsPoint(touchLocationInNode) && player->currentState != player->selectState)
    {
        ...
        if (currentSkillCoolDown <= 0) {
            schedule(CC_SCHEDULE_SELECTOR(FireCombust::updateEnemies), 0.00);
...
void FireCombust::onTouchMoved(Touch* touch, Event* event)
{
...
void FireCombust::onTouchEnded(Touch* touch, Event* event)
{
    if (_skillButton->isPressed) {
        ...
        unschedule(CC_SCHEDULE_SELECTOR(FireCombust::updateEnemies));
    }
}

void FireCombust::performSkill(Vec2 target) {
    ...
    if (currentSkillCoolDown <= 0 && enemies.size() > 0) {
        ...
        schedule(CC_SCHEDULE_SELECTOR(FireCombust::update), 0.05);
    ...
    CCLOG("Nums of Enemies: %d", enemies.size());
    for (auto enemy : enemies) {
        //CCLOG("LULE");
        if (enemy) {
            ...
            //Skill Animate
            Animate* skillAnimate = Animate::create(Engine::createAnimation2("fire_combust", 60, 0.025));
...
void FireCombust::update(float dt) {
    if (currentSkillCoolDown >= 0) {
        ...
        if (coolDownToInt < 0 && coolDownCountLable->isVisible()) {
            ...
            coolDownCountLable->setVisible(false);
            unschedule(CC_SCHEDULE_SELECTOR(FireCombust::update));
        }
...
void FireCombust::updateEnemies(float dt) {
...
void FireCombust::updateEffect(float dt) {
    //if (effectTime >= 0) {
    //    auto player = dynamic_cast<Player*>(this->getParent());
    ...
    //    }
    //}
    //else unschedule(CC_SCHEDULE_SELECTOR(FireCombust::updateEffect));
