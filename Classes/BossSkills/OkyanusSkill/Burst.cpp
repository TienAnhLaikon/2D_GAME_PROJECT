...
bool Burst::init() {
	...
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("skill/SkillSprite/BossSkill/OkyanusSkill/burst/burst.plist", "skill/SkillSprite/BossSkill/OkyanusSkill/burst/burst.png");
	...
	_skillSprite = Sprite::createWithSpriteFrameName("burst (1).png");
	_skillSprite->setAnchorPoint(Vec2(0.5, 0.5));
	_skillSprite->retain();

	_skillAnimate = Animate::create(Engine::createAnimation2("burst", 100, 0.015));
...
void Burst::performSkill(Vec2 target) {
...
void Burst::castRangeIndicator(Vec2 target) {
	auto scene = this->getParent()->getScene();
